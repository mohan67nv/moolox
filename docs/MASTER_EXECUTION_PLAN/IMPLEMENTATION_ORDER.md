...
- **Day 61–64 (`Task 4.1 - Platform Pod & AST Pod`):** Build `packages/git/src/github/*.ts` GitHub App OAuth client (`GIT-01`) and encrypted Personal Access Token (`PAT`) store (`AUTH-04`). Connect repository linking UI in project settings.
- **Day 65–68 (`Task 4.2 - AST Pod [CRITICAL PATH]`):** Build clean Next.js 15 App Router code exporter (`GIT-02`). Verify compiler converts internal AST trees and `tokens.json` into standalone, buildable `/src/app/*` TSX code with zero DIOS runtime imports.
- **Day 69–71 (`Task 4.3 - Platform Pod [CRITICAL PATH]`):** Mount Inngest background event worker (`pushCommitJob`) (`GIT-03`). Verify canvas save triggers background job, bundles code, and commits to linked GitHub branch (`feat(dios): update copy`) in `< 4 seconds`.
- **Day 72–74 (`Task 4.4 - AST Pod`):** Build incoming GitHub push webhook listener (`/api/webhooks/github`) (`GIT-04`). Verify developer edits in VS Code trigger webhook, run SWC parser, and re-render active canvas AST inside `< 3 seconds` without conflict. Build `/dashboard` switcher (`WS-04`, `WS-05`).
...
| **Sprint 10 (`Enterprise`)**<br/>*(Months 10–12 / 60 Days)* | **Enterprise Pod:** `ENT-03` (GDPR eu-west-1), `ENT-04` (SCIM 2.0)<br/>`ENT-05` (AWS KMS CMEK), `ENT-06` (HIPAA Shield)<br/>**Platform Pod:** `AUTH-05` (SAML 2.0 SSO) | Activates `enterprise_orgs` hierarchy. Mounts Okta/Azure AD SAML SSO and SCIM provisioning. Enforces European `eu-west-1` data residency routing and customer AWS KMS envelope encryption (`CMEK`). |
